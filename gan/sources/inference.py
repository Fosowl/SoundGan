...
def spectrogram_to_wav(img, output_path, sr=22050, hop_length=512, n_fft=1024):
    ...
    S_dB = (img * 80.0) - 80.0  # Scale back to original decibel range (-80 to 0 dB)
...
def inference(device, config):
    ...
    plt.figure(figsize=(config.original_image_size[0] / 100, config.original_image_size[1] / 100), dpi=100)  # Adjust DPI
    plt.imshow(img[0], cmap='viridis', aspect='auto')  # Use aspect='auto' to preserve aspect ratio
    plt.axis('off')  # Turn off axis for a cleaner image
    plt.tight_layout(pad=0)
    plt.savefig("output_inference.png", dpi=300, bbox_inches='tight', pad_inches=0)  # Save with high DPI
    plt.close()
